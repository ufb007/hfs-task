<template>
    <div>
        <h3 class="z-100 text-2xl text-white relative mb-5">Create new article</h3>
    ...
    const submit = async () => {
        ...
        try {
            const formData = { 
                ...
                category_id: parseInt(id.value)
            }

            const { status,data } = await axios.post('/articles', formData);

            if (status === 201) {
                window.location.href = `/topics/category/${id.value}/articles`;
            }
        } catch (error) {
            console.error(error);
        }
